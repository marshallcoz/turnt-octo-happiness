...
[res,receptor] = initreceptores(res,m_vars,f_vars,ops);
...
[Bou] = initBoundary(m_vars,ops.N,res);

plotGeom(1,Bou,res,receptor,p0,false);
[xli,yli,zli] = plotGeom(1,Bou,res,receptor,p0,true);
...
%% funciones de Green (espacio estratificado)
[pota] = makepota(receptor,Bou);
for j=10:10%meter a loop en frecuencia
    [m,f] = setupJ(j,m_vars,f_vars,ops);
    % Matriz global invertida en el número de onda 
    [Mpsv,M_sh,mtrcs] = GijTij_HSestr_dwn(m,f,ops);
    % Funciones de Green para espacio estratificado 
    [receptor,Bou] = getGij_lay_psv_dwn(pota,receptor,Bou,...
